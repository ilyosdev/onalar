class Eskiz{
	...
	constructor(email = '', secret = '') {
		...
		if(SMS_GATEWAY_TOKEN == ''){
			sms_gateway_data = JSON.parse(this.sendPing(smsdata, 'auth/login', 'oauth'), 1)
			...
			// print_r(data)
			if(data['message'] != 'authenticated_user' && data['data']['status'] !== 'active'){
	...
	getSMS(sms_id ) { 
		...
		// print_r(output)
		if(output['status'] === 'success' && output['message']['status'] === 'Delivered'  ){
	...
	sendPing(content, method, type = '') { // type is oauth when token is needed
